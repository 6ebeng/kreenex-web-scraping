...
function validate(method) {
	switch (method) {
		case 'search':
			{
				return [check('Url').notEmpty().withMessage('Url field is required').trim()];
			}
			break;
	}
}
			...
			return await page.evaluate(
				async ({ selector, attribute }) => {
					return Array.from(
						(function () {
							var arr = [];
							var results = document.evaluate(selector, document, null, XPathResult.ORDERED_NODE_ITERATOR_TYPE, null);
...
async function search(req, res) {
    ...
    */
		...
		if (Size) {
			...
			var NotInStockSizes = await elementSelector(
				page,
				data.notInStockSizes.selector,
				data.notInStockSizes.attribute || null,
				data.notInStockSizes.regex || null,
				data.notInStockSizes.groups || [],
				true
			);
			var isOutStock;
			...
			if (isInstock) {
				if (data.clickSize) {
					await elementClick(page, data.clickSize.replace('{{size}}', Size.trim()));
					await delay(2000);
				}
			}
		}
		...
		var strCategory =
			(await elementSelector(page, data.category.selector || null, data.category.attribute || null, data.category.regex || null, data.category.groups || [], true)) || '';
